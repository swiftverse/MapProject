...
struct EditView: View {
    @StateObject private var editVM = EditViewVM()
    @Environment(\.dismiss) var dismiss
    var save:(Location) -> Void
    var location: Location
init(location: Location, save: @escaping (Location) -> Void) {
    ...
    var body: some View {
        NavigationView {
            Form {
                Section {
                    TextField("Name of location", text: $editVM.nameLocation)
                    TextField("Description", text: $editVM.descriptionLocation)
                }
                
                Section("Near by places...") {
                    switch editVM.loadingState {
                    case .loaded:
                        ForEach(editVM.pages, id: \.pageid) {
                            page in
                            Text(page.title)
                                .font(.headline)
                            + Text(": ") +
            ...
            .toolbar {
                Button("Save") {
                 ...
                 //   location.name = nameLocation
                    var newLocation = location
                    newLocation.id = UUID()
                    newLocation.name = editVM.nameLocation
                    newLocation.description = editVM.descriptionLocation
                    save(newLocation)
                    dismiss()
                }
            }
    ...
    @MainActor   func nearByPlaces() async {
        editVM.nameLocation = location.name
        editVM.descriptionLocation = location.description
        ...
        do {
            let (data, _) = try await URLSession.shared.data(from: url)
            let items = try JSONDecoder().decode(Result.self, from: data)
            editVM.pages = items.query.pages.values.sorted()
            editVM.loadingState = .loaded
        }
        
        catch {
            editVM.loadingState = .failed
        }
    }
